...
def section_zero_prepare_song_cost():
    ...
    with st.expander("사용자 가이드"):
        st.markdown(
            """
            [사용안내]
            1. 가장 먼저 'input_song cost' 시트에서 이번 달(YYYYMM) 탭을 열고, '당월 발생액' 값을 업데이트 해주세요.
            2. 프로그램의 "곡비 파일 수정하기" 버튼을 누르면, 직전 달 전월 잔액과 이번 달 소속별 매출(UMAG, FLUXUS)을 참조하여 해당 아티스트의 전월 잔액, 당월 차감액, 당월 잔액이 자동 업데이트 됩니다.
            3. 만약 'input_online revenue'에서 아티스트 매핑이 안 된 경우, 결과 화면에서 '누락 행 목록'을 확인하세요.
            """
        )
...
def section_one_report_input():
    ...
    with st.expander("사용자 가이드"):
        st.markdown(
            ...
            2. '정산 보고서 생성 시작' 버튼을 누르면, 'input_song cost'와 'input_online revenue' 데이터를 조회하여 새로운 구글 스프레드시트가 생성됩니다.
            3. 생성된 스프레드시트에는 각 소속의 아티스트별 (정산서/세부매출내역) 탭이 자동 생성되며, 각종 수치(곡비, 매출, 공제 금액, 잔액 등)가 반영됩니다.
            4. 작업이 완료되면, 'input_song cost' 시트에서 다음달 탭이 자동 생성됩니다. (전월 잔액 값만 자동 기입됩니다.)
